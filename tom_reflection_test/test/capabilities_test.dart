...
library test_reflection.test.capabilities_test;

import 'package:test/test.dart';
import 'package:tom_reflection/tom_reflection.dart' as r;
import 'capabilities_test.reflection.dart';

class StaticReflector extends r.Reflection {
  const StaticReflector()
    : super(
        const r.StaticInvokeCapability('foo'),
        const r.StaticInvokeCapability('getFoo'),
        const r.StaticInvokeCapability('setFoo='),
        const r.StaticInvokeCapability('nonExisting'),
        const r.StaticInvokeMetaCapability(C),
...
class InstanceReflector extends r.Reflection {
  const InstanceReflector()
    : super(
        const r.InstanceInvokeCapability('foo'),
        const r.InstanceInvokeCapability('getFoo'),
        const r.InstanceInvokeCapability('setFoo='),
        const r.InstanceInvokeCapability('nonExisting'),
        const r.InstanceInvokeMetaCapability(C),
        r.declarationsCapability,
      );
}
...
void testDynamic(B o, String description) {
  test('Dynamic invocation $description', () {
    ...
    expect(() => instanceMirror.invoke('bar', []), throwsReflectionNoMethod);
...
void main() {
  ...
  test('Static invocation', () {
    ...
    expect(() => classMirror.invoke('bar', []), throwsReflectionNoMethod);
    expect(classMirror.invokeGetter('getFoo'), 44);
    expect(() => classMirror.invokeGetter('getBar'), throwsReflectionNoMethod);
    expect(A.field, 46);
