...
library test_reflection.test.basic_test;

import 'package:test/test.dart';
import 'package:tom_reflection/tom_reflection.dart' as r;
import 'basic_test.reflection.dart';
...
Matcher throwsReflectionNoSuchMethod = throwsA(isReflectionNoSuchMethodError);
Matcher isReflectionNoSuchMethodError =
...
void main() {
  ...
  var reflection = const MyReflection();

  r.ClassMirror aClass = reflection.reflectType(A) as r.ClassMirror;
  r.ClassMirror bClass = reflection.reflectType(B) as r.ClassMirror;
  r.ClassMirror cClass = reflection.reflectType(C) as r.ClassMirror;

  r.InstanceMirror aM = reflection.reflect(aClass.newInstance(unnamed, []));
  r.InstanceMirror bM = reflection.reflect(bClass.newInstance(unnamed, []));
  r.InstanceMirror cM = reflection.reflect(cClass.newInstance(unnamed, []));
  ...
  test('bMethod', () {
    expect(() => aM.invoke(bMethod, []), throwsReflectionNoSuchMethod);
    expect(bM.invoke(bMethod, []), 'bMethod');
    expect(cM.invoke(bMethod, []), 'bMethod');
  });

  test('cMethod', () {
    expect(() => aM.invoke(cMethod, []), throwsReflectionNoSuchMethod);
    expect(() => bM.invoke(cMethod, []), throwsReflectionNoSuchMethod);
