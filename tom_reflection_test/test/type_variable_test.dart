...
library test_reflection.test.type_variable_test;

import 'package:tom_reflection/tom_reflection.dart';
import 'package:test/test.dart';
import 'type_variable_test.reflection.dart';

class NoTypeVariablesReflector extends Reflection {
  const NoTypeVariablesReflector()
    : super(libraryCapability, typeRelationsCapability);
}

class NoBoundsReflector extends Reflection {
  const NoBoundsReflector()
    : super(declarationsCapability, typeRelationsCapability, libraryCapability);
}
...
const Reflection noTypeVariablesReflector = NoTypeVariablesReflector();
const Reflection noBoundsReflector = NoBoundsReflector();
...
void runTestNoBounds(String message, ClassMirror classMirror) {
  test('Type variables, no bounds capability, $message', () {
    ...
    expect(
      xMirror.qualifiedName,
      'test_reflection.test.type_variable_test.B.X',
    );
    ...
    expect(
      yMirror.qualifiedName,
      'test_reflection.test.type_variable_test.B.Y',
    );
...
void runTest(
  ...
  test('Type variables, $message', () {
    ...
    expect(
      dParameterMirror.upperBound.originalDeclaration,
...
void main() {
  ...
  var noBoundsLibraryMirror = noBoundsReflector.findLibrary(
    'test_reflection.test.type_variable_test',
  );
  runTestNoBounds(
    'generic class',
    noBoundsLibraryMirror.declarations['B'] as ClassMirror,
  );
  runTestNoBounds(
    'instantiated generic class',
    noBoundsReflector.reflect(b).type,
  );
