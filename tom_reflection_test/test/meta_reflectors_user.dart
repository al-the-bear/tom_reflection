...
Iterable<String> getNames(Iterable<Reflection> reflections) {
  return reflections.map((Reflection reflector) {
...
void runTests() {
  ...
  test('MetaReflector, set of reflectors', () {
    ...
    expect(
      getNames(const ScopeMetaReflector().reflectionsOfScope('polymer')),
      {'Reflector', 'ReflectorUpwardsClosed'},
    );
    expect(
      getNames(const ScopeMetaReflector().reflectionsOfScope('observe')),
      {'Reflector2', 'ReflectorUpwardsClosed'},
    );
  });

  const ScopeMetaReflector()
      .reflectionsOfScope('polymer')
      .forEach(
  ...
  test('MetaReflector, select by capability', () {
    var reflector = reflectors.firstWhere((Reflection reflector) {
