...
/// to private features.
library test_reflection.test.private_class_test;

@GlobalQuantifyCapability('PublicClass', privacyReflection)
import 'package:tom_reflection/tom_reflection.dart';
import 'package:test/test.dart';
import 'private_class_library.dart';
import 'private_class_test.reflection.dart';

// ignore_for_file: omit_local_variable_types

class PrivacyReflection extends Reflection {
  const PrivacyReflection()
    : super(
...
const privacyReflection = PrivacyReflection();
...
void testPrivacyViolation(
  ...
  test('Privacy, $description', () {
    var canReflect = privacyReflection.canReflect(object);
    expect(canReflect, doReflect);
    if (canReflect) {
      // Check that we can reflect upon [object], and that its class
      // is among the expected ones.
      InstanceMirror instanceMirror = privacyReflection.reflect(object);
...
void main() {
  ...
  test('Privacy, libraries', () {
    ...
    Map<Uri, LibraryMirror> libraries = privacyReflection.libraries;
